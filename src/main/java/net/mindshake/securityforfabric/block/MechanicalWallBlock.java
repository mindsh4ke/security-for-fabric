...
public class MechanicalWallBlock extends BlockWithEntity {

    public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;

    public static final BooleanProperty POWERED = Properties.POWERED;

    public MechanicalWallBlock(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new MechanicalWallBlockEntity(pos, state);
    }
    ...
    public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        if (state.get(POWERED)) {
            return Block.createCuboidShape(0,0,4,16,48,12);
        } else {
            return Block.createCuboidShape(0,0,4,16,16,12);
        }
    }
